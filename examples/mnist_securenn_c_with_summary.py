...
def train():
  ...
  with tf.name_scope('layer1_bf_relu'):
      layer1_bf_relu = conv2d(x_image, Wconv1, STRIDE) + bconv1
      tf.summary.histogram('layer1_bf_relu', layer1_bf_relu)
  layer1 = pooling(tf.nn.relu(layer1_bf_relu))

  with tf.name_scope('layer2_bf_relu'):
      layer2_bf_relu = conv2d(layer1, Wconv2, STRIDE) + bconv2
      tf.summary.histogram('layer2_bf_relu', layer2_bf_relu)
  layer2 = pooling(tf.nn.relu(layer2_bf_relu))
  layer2 = tf.reshape(layer2, [-1, HIDDEN_FC1])

  with tf.name_scope('layer3_bf_relu'):
      layer3_bf_relu = tf.matmul(layer2, Wfc1) + bfc1
      tf.summary.histogram('layer3_bf_relu', layer3_bf_relu)
  layer3 = tf.nn.relu(layer3_bf_relu)

  with tf.name_scope('layer4_bf_relu'):
      layer4_bf_relu = tf.matmul(layer3, Wfc2) + bfc2
      tf.summary.histogram('layer4_bf_relu', layer4_bf_relu)
  layer4 = tf.nn.relu(layer4_bf_relu)
